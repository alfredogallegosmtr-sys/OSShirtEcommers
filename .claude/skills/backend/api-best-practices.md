...
**1. URL Path (Recomendado):** `GET /api/v1/users`, `GET /api/v2/users`
**2. Query Parameter:** `GET /api/users?version=1`
**3. Header:** `Accept: application/vnd.myapi.v1+json`
**4. Content Negotiation:** `Accept: application/vnd.myapi+json; version=1`
...
{
  "data": [...],
  "pagination": { "nextCursor": "def456", "prevCursor": "xyz789", "hasNext": true }
}
```
...
- **JWT Bearer Token:** `Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...`
...
Estrategias: Fixed Window, Sliding Window, Token Bucket, Leaky Bucket.
...
Cache-Control: private, max-age=300                    # Private (browser only)
...
Response:
{
  "status": "healthy",
  "version": "1.2.3",
  "checks": { "database": "healthy", "cache": "healthy", "externalApi": "degraded" },
  "uptime": 86400
}
```
