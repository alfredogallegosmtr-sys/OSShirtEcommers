...
|---|---|---|---|
| E1 | Persistencia real de checkout (direcciones, pagos, pedidos) | **Cerrado (2026-09-01)** — F-01/F-02/F-03/A-01/B-23, sin items pendientes (`B-23` reabrió el epic: `createOrder` sin transacción, encontrado al redactar el guion de defensa técnica) | _(pendiente)_ |
...
| E5 | Limpieza de bugs y código muerto detectados en la auditoría | **Abierto (2026-09-01)** — B-01 a B-21 cerrados; **`B-22` queda pendiente** (reset optimista de `AddressForm.jsx`/`PaymentForm.jsx`, encontrado al implementar `B-21` pero fuera de su alcance) | _(pendiente)_ |
...
| E10 | Despliegue a Render | **Cerrado (2026-08-31)** — DEP-01 | _(pendiente)_ |
...
| E12 | Auditoría de seguridad OWASP Top 10:2025 | **Cerrado (2026-09-01)** — `S-05` a `S-12` corregidos uno por uno, cada uno con rama/PR propio (`S-12` reabrió el epic: gap encontrado al verificar un checklist de pruebas de seguridad pasado por el usuario contra el código real) | _(pendiente)_ |
| E13 | Performance frontend: code-splitting de rutas | **Cerrado (2026-08-31)** — PERF-01, PERF-02 | _(pendiente)_ |
| E14 | Arquitectura frontend: hooks reutilizables | **Cerrado (2026-09-01)** — ARCH-01 | _(pendiente)_ |
...
| DOC-01 | Escribir specs por épica en `docs/specs/` a medida que cada una arranque | E1–E14 | Documentación | **Medio** | **Cerrado (2026-09-01)** — decisión consciente de no adoptar el flujo spec-first: `E9` a `E14` (5 épicas completas, cerradas después de que quedó preparada la infraestructura) se implementaron sin spec previo, con el mismo patrón real de todo el proyecto (1 pendiente → 1 rama → 1 PR, verificado en vivo antes de mergear). El spec de `E10` (único real, `DRAFT`) queda como fue, sin reconstruir specs retroactivos para el resto — ver `docs/specs/README.md` |
...
| B-18 | Causa raíz más profunda de B-16, sin cerrar del todo por ese fix: `cart.controller.js` (`updateQuantity`) persistía "última escritura del servidor gana", no "último clic del usuario gana" — si el PATCH del clic más viejo llegaba después al servidor, el valor persistido en Mongo quedaba desincronizado del que mostraba la UI (B-16 solo protegía el estado mostrado, no lo guardado) | E5 | Bug/Deuda de diseño | **Alto** | **Cerrado (2026-08-31)** — `clientTimestamp` (capturado en el clic) + `findOneAndUpdate` atómico con `$elemMatch`/`$lte`; PATCH más viejo que el último aplicado se descarta sin error |
...
| OBS-02 | Correr Artillery contra la API real en Render (producción, Atlas free tier) | E9 | Observabilidad | **Bajo** | **Cerrado (2026-08-31)** — `ecommerce-api/loadtest/catalog.prod.yml` + `npm run test:load:prod`, carga deliberadamente ligera (2 arrivals/seg, solo catálogo público) para no forzar el free tier. Hallazgo real: el primer intento con el servicio dormido (free tier de Render se duerme tras inactividad) falló 40/40 por timeout; con el servicio despierto, 40/40 exitosos (p95 1.35s, p99 1.6s) |
| DEP-02 | Build Command de Render instalaba devDependencies (`npm install` a secas) — build más lento y `npm audit` reportaba 15 vulnerabilidades moderadas de `artillery`/`@opentelemetry` que nunca corren en producción | E10 | Deuda técnica | **Bajo** | **Cerrado (2026-09-01)** — Build Command cambiado a `npm install --omit=dev` en el dashboard de Render; verificado en vivo (`0 vulnerabilities`, API/CORS/imágenes/docs siguen funcionando tras el redeploy) |
| DEP-03 | Probar en vivo que el rollback de Render funciona (nunca se había verificado) | E10 | Observabilidad | **Bajo** | **Cerrado (2026-09-01)** — rollback real al primer deploy (pre-sesión, sin `helmet`/rate limiting/Swagger): confirmado que perdió esos headers/rutas. Roll-forward al deploy actual: confirmado que volvieron. La base de datos (Atlas) nunca se tocó en ninguna dirección — un rollback solo revierte código, nunca datos |
| ARCH-01 | Extraer `useFetch` (regla de las 3 repeticiones) tras evaluar con evidencia real 3 hooks candidatos | E14 | Refactor | **Bajo** | **Cerrado (2026-09-01)** — `useFetch` extraído (8 componentes migrados, confirmado por lectura directa que el esqueleto se repetía igual en los 8); `useCart` ya existía; `useFormReducer` evaluado y descartado (3 estrategias de validación + 2 formas de estado distintas entre los 4 formularios reales — forzarlo violaría la propia regla de las 3 repeticiones). Bug real encontrado y corregido durante la migración: mover `selectedOrderId` de `Orders.jsx` a un efecto separado rompía `Orders.test.jsx` (ciclo de render extra) — se corrigió como valor derivado en el render |
| DEP-01 | Crear servicios en Render (Web Service + Static Site), migrar datos a Atlas y desplegar | E10 | Deuda técnica | **Bajo** | **Cerrado (2026-08-31)** — backend y frontend reales en Render, Atlas migrado (298 documentos), CORS/imágenes/headers de seguridad verificados en vivo. Auto-Deploy nativo de Render sobre `main` (no Deploy Hooks vía GitHub Actions — esa alternativa queda documentada en `docs/render-deployment.md` como mejora futura) |
...
| S-12 | `login`/`register` (`auth.controller.js`) validaban `email`/`password` solo por truthiness, no por tipo — un payload con esos campos como objeto (intento de inyección NoSQL, ej. `{"$gt":""}`) o un `Content-Type` que `express.json()` no parsea reventaban con 500 en vez de un 422 controlado | E12 | Bug/Seguridad | **Medio** | **Cerrado (2026-09-01)** — [PR #22](https://github.com/alfredogallegosmtr-sys/OSShirtEcommers/pull/22), verificado en vivo contra producción antes y después del fix; sin bypass de autenticación en ningún momento, el problema era la respuesta (500 no controlado), no la seguridad de la auth en sí |
| B-19 | `Header` (monta `Navigation`: fetch + lógica derivada de categorías en cada ruta) no tenía ningún `ErrorBoundary` — el único boundary real del proyecto (`PERF-01`) envuelve `<Routes>` en `App.jsx`, no `<Layout>`, así que un error de render en `Header` tumbaba toda la app, incluido el contenido de la página ya protegido | E5 | Bug | **Medio** | **Cerrado (2026-09-01)** — [PR #23](https://github.com/alfredogallegosmtr-sys/OSShirtEcommers/pull/23), `Layout.jsx` envuelve solo `<Header />` en su propio `ErrorBoundary` (reusa el componente existente); `Footer` queda fuera por ser puramente presentacional |
| DOC-04 | `docs/runbooks/` solo tenía el índice (`README.md`), sin ningún procedimiento real escrito — faltaba documentar cómo diagnosticar un fallo en producción a partir de los logs de Render | — | Documentación | **Bajo** | **Cerrado (2026-09-01)** — [render-logs-diagnostico.md](./runbooks/render-logs-diagnostico.md): qué mirar primero, tabla síntoma→causa real de este backend, y el caso real ya vivido de `ENABLE_DOCS` (404 de `/api-docs` que casi se diagnosticó mal como deploy viejo) |
| B-20 | Favicon default de Create React App (no la marca real del sitio) + recargar cualquier ruta que no fuera `/` (u otra navegación directa por URL) daba "Not Found" en producción — el Static Site de Render sirve archivos reales por default, `GET /product/123` no existe como archivo | — | Bug | **Medio** | **Cerrado (2026-09-01)** — [PR #27](https://github.com/alfredogallegosmtr-sys/OSShirtEcommers/pull/27): `favicon.svg` nuevo replica el sello (hanko) con el kanji `服` que ya usa el `Header` como logo. El SPA rewrite **no** se resolvió con el `public/_redirects` de ese PR (Render no lee ese archivo, a diferencia de Netlify — verificado en vivo que no tenía ningún efecto, y confirmado contra la documentación real de Render) — la corrección real fue una regla en el dashboard del servicio, pestaña "Redirects/Rewrites": Source `/*` → Destination `/index.html` → Action `Rewrite`. Verificado en vivo: `/orders`, `/product/:id`, `/settings` responden 200 al recargar. **Detalle importante:** esa regla vive solo en la configuración del servicio de Render, no en el repo — si el Static Site se recrea desde cero, hay que volver a agregarla a mano (ver `docs/render-deployment.md`) |
| B-21 | `Checkout.jsx` y `WishList.jsx` compartían el mismo estado de error entre la carga inicial (bloqueante, correcta) y acciones posteriores que no deberían bloquear la página — en `Checkout.jsx`, 5 acciones (crear orden, guardar/eliminar dirección, guardar/eliminar método de pago) reemplazaban **todo** el checkout con una línea de texto sin retry al fallar; en `WishList.jsx`, un fallo al quitar un producto (`removeError`) borraba toda la grilla de favoritos igual que un fallo de carga real | E5 | Bug | **Alto** | **Cerrado (2026-09-01)** — estados de error separados por acción (`orderError`/`addressActionError`/`paymentActionError` en Checkout, `removeError` ya no se mezcla con `fetchError` en WishList), renderizados inline en la sección donde ocurre el fallo. Encontrado al correr 2 simulacros reales de caída de base de datos contra producción y comparar contra una referencia externa de "fallbacks dignos" |
| UX-01 | `Home.jsx`/`ProductDetails.jsx`/`SearchResultsList.jsx` mostraban "Revisa tu conexión a internet" para cualquier fallo de red, incluso cuando el problema era 100% del servidor (confirmado en vivo en 2 simulacros de caída de Atlas); `CategoryProducts.jsx` mostraba "Categoría no encontrada" para **cualquier** error (red, servidor), no solo un 404 real; ningún mensaje de error del proyecto decía qué seguía funcionando mientras algo fallaba | — | Deuda de UX | **Medio** | **Cerrado (2026-09-01)** — mensajes honestos (sin culpar la conexión del usuario cuando no se sabe la causa), con contexto de impacto ("tu carrito y tu sesión no se vieron afectados") y botón de retry en `Home`/`ProductDetails`/`SearchResultsList`/`CategoryProducts`/`WishList`/`Orders`/`Profile`. `CategoryProducts.jsx` separa `NOT_FOUND` (404 real) de cualquier otro error. `Navigation.jsx` evaluado y dejado igual: un fallo de categorías no bloquea ninguna acción real del usuario |
| B-22 | `AddressForm.jsx` y `PaymentForm.jsx` resetean el formulario a vacío de inmediato al hacer submit (creación), sin esperar la respuesta real de la API — si el guardado falla del lado del servidor, los datos que el usuario acababa de escribir ya no están para reintentar editarlos, solo para volver a escribirlos desde cero | E5 | Bug | **Bajo** | **Abierto (2026-09-01)** — encontrado al implementar `UX-01`/`B-21` (se verificó antes de prometer en el mensaje de error que "tus datos no se perdieron", promesa que hubiera sido falsa). No corregido en ese PR: el fix real espera a que `onSubmit` resuelva con éxito antes de resetear, cambio de comportamiento async distinto al alcance de esa tarea |
| B-23 | `createOrder` (`order.controller.js`) hacía 3 escrituras seguidas sin transacción: descontar stock → crear la orden → vaciar el carrito. Un fallo de conexión a Atlas justo entre la primera y la segunda dejaba el stock ya descontado sin que existiera ninguna orden — nada lo revertía | E1 | Bug/Deuda de diseño | **Alto** | **Cerrado (2026-09-01)** — envuelto en `session.withTransaction()` (Mongoose): las 3 escrituras comparten sesión, cualquier fallo revierte todo junto, sin rollback manual escrito a mano. El caso de stock insuficiente pasó de un `return` directo del controller a una `InsufficientStockError` personalizada atrapada en el error handler global (`src/app.js`), mismo patrón que el duplicado de índice (`code: 11000`) — respeta la convención del repo de controllers sin `try/catch`. Hallazgo encontrado al redactar una respuesta honesta para el guion de defensa técnica sobre "¿qué pasa si Atlas cae en checkout?", no durante una auditoría formal. **Detalle no obvio:** `mongodb-memory-server` arranca en modo standalone por default, que rechaza transacciones (`"Transaction numbers are only allowed on a replica set member"`) — `tests/integration/helpers/db.js` (compartido por los 24 archivos de test de integración) cambió de `MongoMemoryServer` a `MongoMemoryReplSet` de 1 nodo. Verificado: 180/180 tests backend en verde, incluido el test ya existente de "carrito con 2 productos y el segundo sin stock → revierte el descuento del primero" (ahora corriendo contra una transacción real, no un rollback manual) |
